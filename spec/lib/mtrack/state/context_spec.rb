...
describe MTrack::State::Context do
  ...
  context "already in use" do
    ...
    it "is not empty" do
      ...
      expect(subject.undefined).to match_array(sample_undefined)
    end
    ...
    it "does not modify internal state through attribute readers" do
      ...
      expect(subject.undefined.add(:v)).to match_array(sample_undefined + [:v])
      expect(subject.tracked).to match_array(sample_tracked)
      expect(subject.undefined).to match_array(sample_undefined)
    end
    ...
    it "adds undefined values" do
      ...
      expect(subject.undefined).to match_array(sample_undefined + [:v])
    end
    ...
    it "deletes undefined values" do
      ...
      expect(subject.undefined).to match_array(sample_undefined - [:und_2])
    end
